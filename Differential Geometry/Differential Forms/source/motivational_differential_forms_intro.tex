...
\end{equation*}
Here, $\nabla_\mathbf{v}\mathbf{f}$ is the directional derivative of $\mathbf{f}$ along some vector $\mathbf{v}$ multiplied by $\norm{\mathbf{v}}$.
This is a weaker concept of the derivative than what we were previously using.
If function $\mathbf{f}$ is differentiable at a point $\mathbf{f}$ (the Jacobian exists), then all directional derivatives at that point will exist.
The converse of this statement is not true.
The relationship between the directional derivative and the Jacobian is that $\mathbf{J_f v} = \nabla_\mathbf{v}\mathbf{f}(\mathbf{x})$, meaning that $d\mathbf{f}(\mathbf{x}, d\mathbf{x}) = \nabla_\mathbf{v}\mathbf{f}(\mathbf{x})$.
...
This is not what people are usually taught when they first learn about shapes, which is reasonable, but it is true.
...
Putting this together, we have the differential $D\mathbf{f_x} \colon T_\mathbf{x}\R^m \to T_{\mathbf{f}(\mathbf{x})}\R^n$ defined by $D\mathbf{f}(\Delta\mathbf{x}) = \mathbf{J_f} \Delta \mathbf{x}$.
...
Here, we have shifted our perspective from the function $f$ in $\mathbf{f}$ in $\nabla_\mathbf{v}\mathbf{f}$ to the vector $\mathbf{v}$.
...
Given this definition, we can create a basis for the tangent space $T_\mathbf{x}\R^n$.
We know that the standard basis for $\R^n$ is $\{\ehat^1, \ldots, \ehat^n\}$, and that the partial derivative operator $\partial_{x^i} = \nabla_{\ehat^i}$.
The partial derivatives give us the change in the directions of $\ehat^i$, so a linear combination of them gives us the change the direction of any vector $\mathbf{v}$.
...
Let us apply this to the simple example of the unit circle, $S^1$, though be aware of the fact that it will not be a formal proof.
...
A minor detail in the notation here is that we usually transition away from boldface symbols for vectors when our focus is no longer on Euclidean vectors (also meaning that we will now use $f$ instead of $\mathbf{f}$).
This is just convention.

More typically, the tangent space is defined using \emph{derivations}.
A derivation is a linear map $D$ that satisfies $D(fg) = gD(f) + fD(g)$, which looks very similar to the product rule for derivatives.
...
\begin{equation*}
    \int_C f(x) \diff x,
\end{equation*}
where $C$ is a curve in $\R^2$ the function $f$ is of the type $C \to \R$.
...
Even if we accept it is valid, our resulting vector would not necessarily be tangent to either $p_1$ or $p_2$.
Thus, our integral is not well-defined.

If we found a way around it, such as (arbitrarily) introducing another function that the tangent vector acts on at each point to return a real number, we would run into another problem.
...
That is nonsensical.
The integral of our function should tell us information that does not depend on the equations we use to describe the manifold.
There are even more issues with this definition.
This does not mean that our definition is not legitimate; it just does not represent what we mean by an integral well.
...
Our idea about tangent vectors is correct, but we cannot directly use it in an integral due to the reasons previously stated.
What we need is a way to measure a given tangent vector and output a real number.
These numbers would then be summed in a `limiting' way to give us an integral.
Thus, our task now is to find objects that input tangent vectors and output real numbers.

If one is familiar with the concept of dual spaces in linear algebra, then the identity of such an object is obvious.
It is an element of the \emph{cotangent space} $T^*_p M$, which is the dual of the tangent space $T_p M$.
We call the elements \emph{cotangent vectors}.
Each point in a manifold has its own cotangent space, and creating a collection of cotangent vectors by picking one for each point gives us a \emph{cotangent field}.
The word `cotangent' denotes that the object is complementary to its tangent counterpart.
Alternative names for all of these concepts involve replacing `cotangent' with `covector'; we will use them interchageably.

Knoweldge of dual spaces, strictly speaking, is not needed to understand or formulate the idea of covectors.
We would have arrived at the same concept by simply thinking of the properties our object needs to have -- being linear functionals.\footnote{A functional is an object that takes a function as input and returns a number. The linearity condition is required for covectors to measure tangent vectors in a `sensible way'. That is, they add and scale the way we want them to.}
It has, however, been helpful to show that such a space containing these objects exists.
Before we try to think of how covectors work with integration, it would be helpful to gain visual intuition on what they are.
...
This is an arbitrary choice, and there are actually an infinite number of level sets (though that would be impossible to draw).
They are close together where the function is steeper, and further apart where the function is less steep.
If the plot represented a horizontal plane, where the elevation is given by $f(x,y)$, we could figure out high counting the number of contour lines we cross.
When we say count, this includes crossing over one by a non-integer amount, e.g. we could be halfway between two lines.

This is exactly how we will visualize covectors.
The above plot can be interpreted as a cotangent field \cite{covector_visualization}.
If we were too `zoom into' a particular point, the covector would be represented by a stack of -- equally spaced -- lines tangent to the contour line at that point.
...
We can see from the diagram below that the covector acts in a linear way, which is what we want.
Also, note that the stack has an orientation (shown by the black line).
This makes sense, since a function increases from one direction, and decreases from the opposite direction.
\begin{figure}[H]
\centering
...
However, it has not been explained why a stack of lines parallel to the contour line of a function at a particular point represents a covector.
It turns out that this does not require anything that we have not covered before.
...
We already know such a map: the differential $Df_p$ of a function $f\colon M \to \R$.

Technically, the differential maps to $T_p \R$ not to $\R$.
...
Due to this association, we can consider the differential to `effectively' map to $\R$.
Since we already know that the differential is linear (from the linearity of derivatives), it follows that it is a covector.
Thus, we already had objects that were covectors.
Furthermore, we know that the differential measures something -- the change in a function due to the change in its inputs.
In this context, we reinterpret it to measure the change of a tangent vector of some manifold instead.
This is perfectly reasonable, since it is a function of tangent vectors.

The reason that the stack lines are tangent to the contour lines, or level sets, is as follows.
Moving along a level set will not change the value of the function, by definition.
Thus, the differential will output zero in this direction.
...
Since the differential is a covector, these two ideas combine to give us the information that the stack lines and level sets are in the same direction as each other.
They are tangent.
Again, note that the actual covector's direction is perpendicular to the stack lines.

We have now developed intuition for covectors after motivating their formulation for the purpose of integration.
It follows that we should now apply them to their intended purpose.
The general idea of integrating covectors does not differ much at all from how integration of a $\R \to \R$ function works.
At some point $p$ on a manifold $M$, the covector $\omega_p$ acts on the tangent vector $v \in T_p M$ to give $\omega_p(v)$.
These measurements are accumulated to give us the result of an integral.
...
How is this done?
We know that the differential $D\phi_p$ is a map from $T_p \R$ to $\T_p M$.
Thus, precomposing it with $\omega_p$ gives us a function $\omega_p \circ D\phi_p \colon T_p \R \to \R$.
...
Given its relation to the differential, it makes sense that an alternative notation for $D\phi$ is $\phi_*$.
...
Our formula for the integral of $\omega$ over a manifold $M$ parameterized by $\gamma: [a,b] \to M$ is
\begin{equation*}
    \int_\gamma \omega \coloneq \int_{[a,b]} \gamma^* \omega.
\end{equation*}
...
We have answered our initial question of what the $dx$ in the integral means, again.\footnote{This is our fourth and final interpretation of what it means.}
...
Precisely, it is a covector field.


% MOVE ON TO THIS AFTER DEVELOPING INTEGRATION FOR ONE FORMS
% We have got (a good portion of) the important part out of the way.
% Now, we will discuss covectors in a more `concrete' manner.
% Firstly, another word for a cotangent field is a \emph{differential 1-form}.
% This is just a name, but it does suggest that there are more types of differential forms.

%TODO EXPLICITLY MENTION THAT THE DIFFERENTIAL IS THE BEST LINEAR APPROXIMATION OF A FUNCTION AT A POINT, FOR A CURVE ITS A LINE, FOR A SURFACE ITS A PLANE, ETC.
